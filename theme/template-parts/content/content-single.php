...
<article id="post-<?php the_ID(); ?>" <?php post_class('max-w-3xl mx-auto px-4 py-16'); ?>>
	...
	<div class="relative h-80 overflow-hidden rounded-2xl mb-10">
		...
		<div class="absolute inset-0" style="background: linear-gradient(to top, rgba(64,72,86,0.6), transparent);"></div>
	</div>
	<?php endif; ?>
	...
	<div class="flex items-center gap-3 mb-6 flex-wrap">
		...
		if ( $cats ) :
		?>
		<span class="bg-iibpr-green text-white text-xs font-bold px-3 py-1 rounded-full">
	...
	<!-- Title -->
	<h1 class="text-3xl md:text-4xl font-extrabold text-iibpr-charcoal mb-8 font-serif">
		<?php the_title(); ?>
	</h1>

	<!-- Content -->
	<div class="prose prose-lg max-w-none prose-iibpr
	...
	<nav class="mt-16 pt-8 border-t border-gray-200">
		<div class="flex justify-between gap-4">
			...
			<a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>" class="flex-1 p-4 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
				...
				<p class="font-semibold text-iibpr-charcoal text-sm mt-1"><?php echo esc_html( $prev_post->post_title ); ?></p>
			</a>
			<?php endif; ?>
			...
			<a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>" class="flex-1 p-4 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors text-right">
				...
				<p class="font-semibold text-iibpr-charcoal text-sm mt-1"><?php echo esc_html( $next_post->post_title ); ?></p>
			</a>
			<?php endif; ?>
		</div>
	</nav>
